...
export default function Dashboard({ refreshTrigger }) {
  const { data, loading, error } = useDados('dashboard', refreshTrigger);
  const [meta, setMeta] = useState(null);
  const [filters, setFilters] = useState({});

  useEffect(() => {
    apiFetch('/config').then(r => r.json()).then(d => setMeta(d.meta_faturamento_mensal)).catch(() => {});
  }, []);
  ...
  const filteredVendedores = applyFilters(topVendedores, {
    Vendedor: filters.Vendedor,
  });

  const filteredMarcas = applyFilters(marcasMes, {
    DescrMarca: filters.DescrMarca,
  });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          label="Documentos"
          value={String(data?.qtd_documentos ?? 0)}
          variant="default"
        />
        <KpiCard
          label="Ticket Médio"
